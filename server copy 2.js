...
function detectWaylandEnv() {
  const dirs = ['/run/user/0', '/run/weston'];
  for (const dir of dirs) {
    try {
      const sock = fs.readdirSync(dir)
                     .find(f => f.startsWith('wayland-'));
      if (sock) {
        return { XDG_RUNTIME_DIR: dir, WAYLAND_DISPLAY: sock };
      }
    } catch { /* dir may not exist – ignore */ }
  }
  throw new Error('Wayland socket not found – is Weston running?');
}
...
/* ── launch Chromium on the given screen id -------------------------------- */
function spawnBrowser(id, url) {
  const port    = DEVTOOLS_PORT[id];
  const profile = path.join(PROFILE_DIR, `chrome${id}`);

  // stop any Chromium that still uses this profile
  try { execSync(`pkill -f -- "--user-data-dir=${profile}"`); } catch {}

  const args = [
    '--kiosk',
    '--ozone-platform=wayland', '--enable-features=UseOzonePlatform',
    '--use-gl=egl', '--use-angle=egl',
    '--enable-gpu-rasterization', '--ignore-gpu-blocklist',
    '--autoplay-policy=no-user-gesture-required',
    '--noerrdialogs', '--disable-infobars', '--disable-session-crashed-bubble',
    `--remote-debugging-port=${port}`,
    `--class=screen${id}`,            // helpful in xdotool / wmctrl
    `--app=${url}`,
    `--user-data-dir=${profile}`
  ];

  /* sudo *must* keep Wayland vars, otherwise Chromium cannot talk to Weston */
  const child = spawn(
    'sudo',
    [
      '--preserve-env=XDG_RUNTIME_DIR,WAYLAND_DISPLAY',
      '-Hu', 'admin',
      BROWSER_BIN,
      ...args
    ],
    {
      env: CHROME_WL_ENV,
      detached: true,
      stdio: ['ignore', 'ignore', 'inherit']   // stdout → journald
    }
  );

  child.unref();
  console.log(`Screen ${id} → ${url}  (profile=${profile})`);
}
...
/*   Weston provides `weston-screenshooter` which always grabs the full      */
/*   virtual display.  We save, optionally down-scale / recompress, then     */
/*   stream the result back.                                                 */
app.get('/screenshot/:id', (req, res) => {
  const id    = req.params.id;                       // still accept 1|2 for API
  ...
  try {
    execSync(`weston-screenshooter ${tmp}`);         // full frame-buffer grab
  } catch (e) {
    console.error(e);
...
/* ============================================================= */
/*  GPU AUTO-CHECK (Pi-specific)                                  */
/*    → http://<pi>:8080/gpu-auto-check                           */
/* ============================================================= */
app.get('/gpu-auto-check', async (req, res) => {
  const adminHome  = '/home/admin';
  const chromeLog  = path.join(adminHome, '.config', 'chromium',
                               'chrome_debug.log');

  /* 1. stop the two kiosk instances so the log we read is ours only */
  try { execSync('pkill -f -- "--class=screen1"'); } catch {}
  try { execSync('pkill -f -- "--class=screen2"'); } catch {}

  /* 2. wipe old debug log (if any)                                 */
  try { fs.unlinkSync(chromeLog); } catch {}

    /* 3. run a throw-away headless Chromium probe (≈ 8 s) */
  const tmpProfile = '/tmp/chrome-probe';
  ...
  const probe = spawn(
    ...
    { env: CHROME_WL_ENV, stdio: 'ignore' }   // keep journald clean
  );

  await new Promise(r => setTimeout(r, 8000));  // allow log to flush
  probe.kill('SIGINT');


  /* 4. helper to grab the first “interesting” line from the log    */
  const firstErr = (() => {
    try {
      return fs.readFileSync(chromeLog, 'utf8')
               .split('\n')
               .find(l => /ERROR|Failed|denied|DRM|GPU/i.test(l))
             || '-- no matching error line --';
    } catch { return '-- chrome_debug.log missing --'; }
  })();
  ...
  const payload = [
    ...
    '',
    'OpenGL renderer (glxinfo -B):',
    safeExec('sudo -Hu admin bash -c "glxinfo -B 2>/dev/null | '
           + 'grep -E \'^OpenGL (vendor|renderer|version)\'"'),
    '',
    'First error line in chrome_debug.log:',
    firstErr,
    '',
 ...
 /* ============================================================= *
 *  GPU QUICK-DUMP
 *    curl http://<pi>:8080/gpu-quick-dump | less
 * ------------------------------------------------------------- */
app.get('/gpu-quick-dump', async (req, res) => {
  const sh = cmd => {
    try { return execSync(cmd, {encoding: 'utf8', maxBuffer: 5e6}).trim(); }
    catch (e) { return (e.stderr || e.message || '(cmd failed)').toString().trim(); }
  };

  /* 1 ── one-shot Chromium (≈ 6 s) just to create chrome_debug.log */
  const adminEnv = {
    ...CHROME_WL_ENV,
    MOZ_DISABLE_CONTENT_SANDBOX: '1',   // keep it small & quiet
  };
  try {
    execSync('pkill -f chrome_debug.log || true');
    spawnSync('sudo', [
      '-E','-Hu','admin', BROWSER_BIN, 'about:blank',
      '--headless','--use-gl=egl','--use-angle=egl',
      '--disable-features=Vulkan','--enable-logging','--v=1',
      '--ozone-platform-hint=auto','--window-size=64,64',
      '--user-data-dir=/tmp/chrome-gpu-dump'
    ], { env: adminEnv, timeout: 6000, stdio: 'ignore' });
  } catch {} /* ignore – we only need the log */

  const chromeLog = '/home/admin/.config/chromium/chrome_debug.log';
  const firstErr  = (() => {
    try { return sh(`grep -m1 -E "ERROR|Failed|denied|DRM|GPU" ${chromeLog}`) || '(none)'; }
    catch { return '(chrome_debug.log missing)'; }
  })();

  /* 2 ── gather everything into one payload */
  const payload = [
    '=== gpu-quick-dump ===========================================',
    `time        : ${new Date().toISOString()}`,
    '',
    '# DRM nodes ---------------------------------------------------',
    sh('ls -l /dev/dri') || '(no /dev/dri)',
    '',
    '# kernel modules ----------------------------------------------',
    sh('lsmod | grep -E "vc4|v3d"') || '(vc4/v3d not loaded)',
    '',
    '# es2_info (EGL renderer) as admin ----------------------------',
    sh('sudo -Hu admin env XDG_RUNTIME_DIR=/run/weston WAYLAND_DISPLAY=wayland-0 es2_info 2>/dev/null | head -n 15') ||
      '(es2_info / mesa-utils-extra missing)',
    '',
    '# first error in chrome_debug.log -----------------------------',
    firstErr,
    '',
    '# id admin / userns knob --------------------------------------',
    'id admin           : ' + sh('id admin'),
    'userns knob        : ' + sh('sysctl -n kernel.unprivileged_userns_clone || echo "(n/a)"'),
    '==============================================================\n'
  ].join('\n');
...
app.get('/webgl-selftest', async (req, res) => {
  ...
  const child = spawn(
    ...
    { env: CHROME_WL_ENV, stdio: ['ignore', 'pipe', 'pipe'] }
  );
